...
def home():
    ...
    if request.method == 'POST':
        ...
        if person_image and allowed_file(person_image.filename) and shirt_image and allowed_file(shirt_image.filename):
            ...
            # Assuming you have found the points "RShoulder" (point 2), "LShoulder" (point 5), and other points
            shoulder2 = points[2]
            shoulder5 = points[5]
            print("hduf\n \n \n")
            ...
            # Get the color at point 5 in the shirt image
            color_at_point5 = shirt_image_resized[shoulder5[1] - y_offset, shoulder5[0] - x_offset, :3]
            
            # Define the points for the polygon based on the specified points
            polygon_points = [points[8], points[4], points[3], points[2], points[0], points[5], points[6],points[6], points[11]]
            # Print the polygon_points to check its contents
            print(polygon_points)

            # Create a mask for the polygon
            mask = np.zeros_like(shirt_image_resized[:, :, 3])
            cv.fillPoly(mask, [np.array(polygon_points)], 255)

            # Overlay the shirt image color onto the person image using the polygon mask
            for c in range(0, 3):
                person_image_with_shirt[y_offset:y_offset + shirt_image_resized.shape[0], x_offset:x_offset + shirt_image_resized.shape[1], c] = \
                    person_image_with_shirt[y_offset:y_offset + shirt_image_resized.shape[0], x_offset:x_offset + shirt_image_resized.shape[1], c] * (1 - mask / 255) + \
                    color_at_point5[c] * (mask / 255)



            # Print values before overlay
            print("Before Overlay:")
            print(person_image_with_shirt[y_offset:y_offset + shirt_image_resized.shape[0], x_offset:x_offset + shirt_image_resized.shape[1], :])
